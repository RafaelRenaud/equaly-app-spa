...
import {
  ...
  CreateUpdateOccur,
} from "../../../../core/model/occur/occur-create-update.model";
...
export class OccurEditComponent implements OnInit {
  ...
  private loadFiles(): void {
    this.fileService
      .getFiles(this.occurId.toString(), "OCCUR", 0, 10)
      .subscribe({
        next: (response) => {
  ...
  private populateForm(occur: Occur): void {
    ...
    if (occur.description)
      this.occurrenceForm.patchValue({ description: occur.description });
    if (occur.complement)
      this.occurrenceForm.patchValue({ complement: occur.complement });

    // Notas fiscais
    if (occur.invoiceNotes?.length) {
      occur.invoiceNotes.forEach((note, index) => {
        if (index < 5)
          this.occurrenceForm.patchValue({ [`nf${index + 1}`]: note });
      });
    }

    // Reclamação
    if (occur.complaint) {
      if (occur.complaint.type)
        this.occurrenceForm.patchValue({ complaintType: occur.complaint.type });
      if (occur.complaint.orderId)
        this.occurrenceForm.patchValue({
          orderNumber: occur.complaint.orderId,
        });
      if (occur.complaint.request?.description) {
        this.occurrenceForm.patchValue({
          complaintDescription: occur.complaint.request.description,
        });
      }
      if (occur.complaint.request?.complement) {
        this.occurrenceForm.patchValue({
          complaintComplement: occur.complaint.request.complement,
        });
      }
      if (occur.complaint.isAnonymous !== undefined) {
        this.occurrenceForm.patchValue({
          anonymousComplainer: occur.complaint.isAnonymous,
        });
      }
      ...
      if (occur.complaint.complainant && !occur.complaint.isAnonymous) {
        ...
        if (c.id) {
          ...
          const internalComplainer = this.buildUserResponse(
            c.id,
            c.name || "",
            c.email || "",
            "ACTIVE",
        ...
        if (c.phone)
          this.occurrenceForm.patchValue({ complainerPhone: c.phone });
        if (c.email)
          this.occurrenceForm.patchValue({ complainerEmail: c.email });
        ...
        if (c.address) {
          ...
          if (addr.street)
            this.occurrenceForm.patchValue({ complainerStreet: addr.street });
          if (addr.number)
            this.occurrenceForm.patchValue({ complainerNumber: addr.number });
          if (addr.district)
            this.occurrenceForm.patchValue({
              complainerDistrict: addr.district,
            });
          if (addr.complement)
            this.occurrenceForm.patchValue({
              complainerAddressComplement: addr.complement,
            });
          if (addr.city)
            this.occurrenceForm.patchValue({ complainerCity: addr.city });
          if (addr.uf)
            this.occurrenceForm.patchValue({ complainerState: addr.uf });
        }
      }
    }
  }
  ...
  onOccurTypeSelected(occurType: OccurTypeResponse | null): void {
    if (occurType) {
      this.occurrenceForm.patchValue({
        occurrenceType: `${occurType.id} - ${occurType.name}`,
      });
  ...
  onInspectorSelected(inspector: UserResponse | null): void {
    if (inspector) {
      this.occurrenceForm.patchValue({
        qualityInspector: `${inspector.id} - ${inspector.username}`,
      });
  ...
  shouldValidateComplainant(): boolean {
    return (
      ...
      !this.occurrenceForm.get("anonymousComplainer")?.value
    );
  }
  ...
  openSaveDraftModal(content: any): void {
    if (!this.isDraft) {
      this.showAlert(
        "WARNING",
        "Apenas rascunhos podem ser salvos como rascunho.",
      );
      return;
    }
  ...
  confirmSaveDraft(): void {
    ...
    this.occurService
      .updateOccur(this.occurId, this.buildOccurData("DRAFT_OPENED"))
      .subscribe({
        next: (response) => {
          this.loadingService.hide();
          this.redirectWithSuccess(
            `Rascunho atualizado com sucesso! Código: ${response.code || this.occurId}`,
          );
        },
        error: () => {
          this.loadingService.hide();
  ...
  confirmUpdateOccurrence(): void {
    ...
    this.uploadProgress = {
      ...
      total: 100,
      status: "uploading",
      message: "Atualizando ocorrência (20%)...",
    };
    this.cdr.detectChanges();

    this.occurService
      .updateOccur(this.occurId, this.buildOccurData("AWAITING_REPORT"))
      .pipe(
        switchMap((response) => {
          this.uploadProgress.current = 20;
          this.uploadProgress.message =
            "Ocorrência atualizada! Processando arquivos...";
          this.cdr.detectChanges();
          return this.uploadFiles().pipe(
            map((uploadResult) => ({ response, ...uploadResult })),
          );
        }),
      )
      .subscribe({
        ...
        error: () => {
          ...
          this.redirectWithError(
            "Erro ao atualizar ocorrência. Tente novamente.",
          );
        },
      });
  }
    ...
    return this.fileService.deleteFiles(this.occurId.toString(), "OCCUR").pipe(
      switchMap(() => {
        ...
        const uploads = this.attachedFiles.map((file, index) =>
          this.fileService
            .createFile(this.occurId.toString(), "OCCUR", file, file.name)
            .pipe(
              map(() => {
      ...
      catchError(() =>
        of({ success: false, failedCount: this.attachedFiles.length }),
      ),
    );
  }
  ...
  private updateComplainantValidators(): void {
    ...
    if (shouldValidate) {
      ...
      [nameCtrl, phoneCtrl, emailCtrl, cepCtrl].forEach((ctrl) =>
  ...
  private buildOccurData(
    ...
    const inspectorId = raw.qualityInspector
      ? parseInt(raw.qualityInspector.split(" - ")[0])
      : undefined;
    const invoiceNotes = [raw.nf1, raw.nf2, raw.nf3, raw.nf4, raw.nf5].filter(
      (n: string) => n?.trim(),
    );
    ...
    if (raw.complaintType && raw.complaintDescription?.trim()) {
      ...
      if (!isAnonymous && this.shouldValidateComplainant()) {
        ...
        if (raw.complainerAddressComplement)
          address.complement = raw.complainerAddressComplement;
        ...
        if (raw.complainerCep)
          address.zipCode = raw.complainerCep.replace(/\D/g, "");
      ...
      complaint = {
        ...
        complainant,
        request: {
          description: raw.complaintDescription,
          complement: raw.complaintComplement || undefined,
        },
      };
    }
  ...
  viewFile(file: FileResponse): void {
    this.loadingService.show();
    this.fileService
      .getFileAccess(file.id!, this.occurId.toString(), "OCCUR", file.hash!)
      .subscribe({
        next: (response) => {
          this.loadingService.hide();
          window.open(
            `${response.url}?access_token=${response.access_token}`,
            "_blank",
          );
        },
        error: () => {
  ...
  onFileSelected(event: Event): void {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (this.attachedFiles.length >= this.maxFiles) {
        this.showAlert(
          "WARNING",
          `Limite de ${this.maxFiles} arquivos atingido`,
        );
        break;
      }
  ...
  private setupFormListeners(): void {
    this.occurrenceForm.get("complaintType")?.valueChanges.subscribe((val) => {
      ...
      this.updateComplainantValidators();
      if (
        this.activeTab === "complainant" &&
        !this.shouldShowComplainantTab()
      ) {
